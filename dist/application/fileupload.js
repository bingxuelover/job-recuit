(function(jQuery) {
  var extendresult = jQuery.extend({
    createUploadIframe: function(id, uri) {
      var frameId = 'jUploadFrame' + id;

      if (window.ActiveXObject && + $.browser.version < 9) {
        var io = $('<iframe name="' + frameId + '" id="' + frameId + '">');
    ...
    ajaxFileUpload: function(s) {
      ...
      try {
        var form = $('#' + formId);
        $(form).attr('action', jQuery.createHttpUrl(s.url, s.data));
    ...
    createHttpUrl: function(orignUrl, data) {
      if (!data) 
        return orignUrl;
      var ra = [],
        regex = /\?/;
      for (var i in data) {
        ra.push(encodeURIComponent(i) + "=" + encodeURIComponent(data[i]));
      }
      var query = ra.join('&');
      return orignUrl + (
        regex.test(orignUrl)
  ...
  if (typeof define !== 'undefined') 
    define('dev/fileupload', [], function() {
      return extendresult
    });
  }
)(jQuery);
